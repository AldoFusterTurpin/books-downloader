...
class DownloadBooksTestSuite(unittest.TestCase):
    ...
    def test_can_reach_all_the_free_books_urls_from_Springer_main_WebPage(self):
        main_webPage_url = "https://link.springer.com/search?facet-content-type=\"Book\"&sortOrder=newestFirst" \
    ...
    def test_simulate_download_of_first_50_books(self):
        main_webPage_url = "https://link.springer.com/search?facet-content-type=\"Book\"&sortOrder=newestFirst&showAll=true&package=mat-covid19_textbooks"

        responses = simulate_download_of_first_n_books(self.driver, main_webPage_url, 50)

        expected_response = 200
        for response in responses:
            self.assertEqual(response, expected_response)

    def test_simulate_download_of_first_200_books(self):
        main_webPage_url = "https://link.springer.com/search?facet-content-type=\"Book\"&sortOrder=newestFirst" \
                           "&showAll=true&package=mat-covid19_textbooks "

        responses = simulate_download_of_first_n_books(self.driver, main_webPage_url, 200)
